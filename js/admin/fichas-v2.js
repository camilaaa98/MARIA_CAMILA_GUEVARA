...
function mostrarFichas(fichas) {
    ...
    tbody.innerHTML = fichasPagina.map(f => {
        ...
        const instructoresDisplay = f.instructores_nombres
            ? `<div style="font-size: 0.9em; margin-bottom: 5px;">${f.instructores_nombres}</div>`
            : '<div style="font-size: 0.9em; color: #666; font-style: italic; margin-bottom: 5px;">Sin asignar</div>';
        ...
        <tr>
            ...
            <td>
                ${instructoresDisplay}
                <button class="btn-instructor" onclick="asignarInstructor('${f.numero_ficha}')" style="font-size: 0.8em; padding: 4px 8px;">
...
function exportarFichas() {
    ...
    let table = `
        <table border="1">
            <thead>
                <tr>
                    <th colspan="4" style="background-color: #39A900; color: white; text-align: center; font-size: 14pt;">
                        AsistNet - Reporte de Fichas
                    </th>
                </tr>
                <tr>
                    <th colspan="4">Fecha de Generación: ${new Date().toLocaleDateString()}</th>
                </tr>
    ...
    datos.forEach(f => {
        table += `
            <tr>
                ...
                <td>${f.instructores_nombres || 'Sin asignar'}</td>
                <td>${f.estado}</td>
            </tr>
        `;
    });
